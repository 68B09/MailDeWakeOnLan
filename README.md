...
・送信先…「２－１」で作ったアドレス  
・件名…何か適当に。「おはよう」でもOK。  
・本文…1行目に合い言葉(magicnumberに設定した文字列)、2行目に「１－２」で調べたクライアントPCのMACアドレス  
(例)  
hogehoge  
00:00:5E:00:53:FF  
...
大きな値、例えば５分などにすると利用者がリクエストしてから５分以上待たされかねないことに注意して下さい。  
また値が大きいと大量のメールが溜まり、メールサーバが送信者にエラーを返すかもしれません。  
...
------
・本番運用の前にテストしましょう  
出勤前にメールを送る→会社に着いたらPCが起動していることを確認する。等。  
